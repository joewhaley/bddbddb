 ...
 * 
 * <P>
 * FSK: right now the implementation tries to preserve the property that if a
 * key 'k' maps to an empty collection 'c' in some MultiMap 'mm', then users of
 * 'mm' will not be able to see that 'k' is a member of the keySet for 'mm'.
 * However, it does not preserve this property when mm.getValues(k) is used as a
 * means to operate on the state of 'mm', and it is not clear to me whether one
 * can even ensure that the property can be maintained if arbitrary operations
 * on mm.getValues(k) are passed on to 'mm'.
 * 
...
public class GenericMultiMap/* <K,V> */implements MultiMap/* <K,V> */, Cloneable {
    ...
    /**
     * Creates a <code>MultiMap</code> using a <code>HashMap</code> for the
     * map and <code>HashSet</code> s for the value collections. To gain more
     * control over the specific sets/map used in internal representation of
     * <code>this</code>, use the more specific
     * {@link GenericMultiMap#GenericMultiMap(MapFactory,CollectionFactory) constructor }
    ...
    /**
     * Creates a <code>MultiMap</code> using a <code>HashMap</code> for the
     * map and the specified <code>CollectionFactory</code> to create the
     * value collections.
     */
     ...
     * 
     * NOTE: I would make this ctor public, but I need to eliminate any issues
     * with the Collection-values being shared between 'this' and 'mm'.
     *  
     */
    ...
    private static int size0(Map internMap) {
        int count = 0;
        for (Iterator/* <Collection <V>> */it = internMap.values().iterator(); it
            .hasNext();)
    ...
    public boolean isEmpty() {
        ...
        for (Iterator/* <Collection <V>> */it = internMap.values().iterator(); it
            .hasNext();)
            if (((Collection) it.next()).size() > 0) return false;
        return true;
    }
    ...
    public boolean containsValue(Object value) {
        for (Iterator/* <K> */it = internMap.keySet().iterator(); it.hasNext();)
            if (getValues(it.next()).contains(value)) return true;
        return false;
    }

    /**
     * Returns some arbitrary value from the set of values to which this map
     * maps the specified key. Returns <code>null</code> if the map contains
     * no mapping for the key; it's also possible that the map explicitly maps
     * the key to <code>null</code>. The <code>containsKey</code> operation
     * may be used to distinquish these two cases.
     * 
     ...
     * map previously contained any mappings for this key, all of the old values
     * are replaced. Returns some value that was previous associated with the
     * specified key, or <code>null</code> if no values were associated
     * previously.
     */
    ...
    /**
     * Removes all mappings for this key from this map if present. Returns some
     * previous value associated with specified key, or <code>null</code> if
    ...
    /**
     * Removes a mapping from key to value from this map if present. Note that
     * if multiple mappings from key to value are permitted by this map, then
     * only one is guaranteed to be removed. Returns true if <code>this</code>
     * was modified as a result of this operation, else returns false.
     */
    ...
    /**
     * Copies the mappings from the specified map to this map. These mappings
     * will replace any mappings that this map had for any of the keys currently
     * in the specified map. Note that <code>putAll(mm)</code> where
     * <code>mm</code> is a <code>MultiMap</code> will NOT add all of the
     * mappings in <code>mm</code>; it will only add all of the Keys in
    ...
    public static boolean add0(Object/* K */key, Object/* V */value,
        Map internMap, CollectionFactory cf) {
     ...
     * with <code>key</code>, constructs a new, mutable, empty
    ...
    private static Collection/* <V> */getValues0(final Object/* K */key,
        Map internMap, CollectionFactory cf) {
        Collection/* <V> */c = (Collection) internMap.get(key);
        if (c == null) {
            c = cf.makeCollection();
    ...
    /**
     * Returns a set view of the mappings contained in this map. This view is
     * fully modifiable; the elements are <code>Map.Entry</code>s. The
     * returned set is actually a <code>MultiMapSet</code>, from which you
     * can get back the original <code>MultiMap</code>.
     */
    ...
    class ValuesCollection extends AbstractCollection/* <V> */{
        ...
        public Iterator/* <V> */iterator() {
            return new Iterator/* <V> */() {
                final Iterator/* <Map.Entry <K,V>> */it = GenericMultiMap.this
                    .entrySet().iterator();
    ...
    // common code
    public static class GenericMultiMapEntrySet extends AbstractCollection/*
                                                                           * <Map.Entry
                                                                           * <K,V>>
                                                                           */
    implements MultiMapSet/* <K,V> */{
        ...
        public MultiMap/* <K,V> */asMultiMap() {
            return new GenericMultiMap(internMap);
        }
        ...
        public boolean equals(Object o) {
            if (o == this) return true;
                ...
                && ((Set) o).size() == GenericMultiMapEntrySet.this.size()) return GenericMultiMapEntrySet.this
                .containsAll((Collection/* <Map.Entry <K,V>> */) o);
            else return false;
        }
    ...
    public static class GenericMultiMapIterator implements Iterator/*
                                                                    * <Map.Entry
                                                                    * <K,V>>
                                                                    */{
        ...
        public Object/* Map.Entry <K,V> */next() {
            ...
            return new PairMapEntry/* <K,V> */(k, o) {
                public Object/* V */setValue(Object/* V */newValue) {
                    if (backing_map == null) throw new UnsupportedOperationException();
                    Object/* V */oldValue = getValue();
                    backing_map.remove(getKey(), oldValue);
                    backing_map.add(getKey(), newValue);

...
public class SCCPathNumbering extends PathNumbering {
    ...
    /**
     * Counts the number of paths from the given root set, using the given graph
     * navigator.
     */
    public BigInteger countPaths(Collection roots, Navigator navigator,
        Map initialMap) {
        ...
        if (TRACE_NUMBERING) System.out.println("Max SCC=" + max_scc
            + ", Num SCC=" + num_scc);
        ...
        while (scc != null) {
            ...
            if (TRACE_NUMBERING) System.out.println("Visiting SCC"
                + scc.getId() + (scc.isLoop() ? " (loop)" : " (non-loop)"));
            BigInteger total = BigInteger.ZERO;
            if (initialMap != null && !scc.isLoop()) total = toBigInt((Number) initialMap
                .get(scc.nodes()[0]));
            ...
            for (Iterator i = Arrays.asList(scc.prev()).iterator(); i.hasNext();) {
                ...
                //System.out.println("Visiting edge SCC"+pred.getId()+" to
                // SCC"+scc.getId());
                ...
                BigInteger newtotal = total.add(t1.multiply(BigInteger
                    .valueOf(nedges)));
            ...
            if (scc.prevLength() == 0) total = total.add(BigInteger.ONE);
            Range r = new Range(fromBigInt(total), fromBigInt(total
                .subtract(BigInteger.ONE)));
            if (TRACE_NUMBERING
                || (PRINT_BIGGEST && total.compareTo(max_paths) == 1)) System.out
                .println("Paths to SCC" + scc.getId()
                    + (scc.isLoop() ? " (loop)" : " (non-loop)") + "=" + total);
        ...
        while (scc != null) {
            ...
            if (TRACE_NUMBERING) System.out.println("Range for SCC"
                + scc.getId() + (scc.isLoop() ? " (loop)" : " (non-loop)")
                + "=" + r);
    ...
    private void recordEdgesFromSCC(SCComponent scc1) {
        ...
        for (int i = 0; i < nodes.length; ++i) {
            ...
            for (Iterator j = targets.iterator(); j.hasNext();) {
                ...
                if (TRACE_NUMBERING) System.out.println("Edge SCC"
                    + scc1.getId() + " to SCC" + scc2.getId() + ": " + target);
    ...
    private void addEdges(SCComponent scc1) {
        if (TRACE_NUMBERING) System.out.println("Adding edges SCC"
            + scc1.getId());
        Range r1 = (Range) sccNumbering.get(scc1);
        if (scc1.prevLength() == 0) {
            if (TRACE_NUMBERING) System.out.println("SCC" + scc1.getId()
                + " is in the root set");
            if (VERIFY_ASSERTIONS) Assert._assert(r1.low.longValue() == 1L
                && r1.high.longValue() == 0L);
            r1.low = new Integer(0);
        }
        if (scc1.isLoop()) {
            Collection internalEdges = (Collection) sccEdges.get(new Pair(scc1,
                scc1));
            for (Iterator i = internalEdges.iterator(); i.hasNext();) {
                Pair edge = (Pair) i.next();
                if (TRACE_NUMBERING) System.out.println("Range for " + edge
                    + " = " + r1);
        ...
        for (Iterator i = Arrays.asList(scc1.next()).iterator(); i.hasNext();) {
            ...
            for (Iterator k = calls.iterator(); k.hasNext();) {
                ...
                Range r3;
                if (important) {
                    // external call. update internal object and make new
                    // object.
                    BigInteger newlow = toBigInt(r2.low).subtract(
                        toBigInt(r1.high).add(BigInteger.ONE));
                    Assert._assert(newlow.signum() != -1);
                    r2.low = fromBigInt(newlow);
                    if (TRACE_NUMBERING) System.out
                        .println("External edge!  New range for SCC"
                            + scc2.getId() + " = " + r2);
    ...
    /**
     * Comparator used to put nodes in post order according to the SCC post
     * order.
     */
    static class PostOrderComparator implements Comparator {
        ...
        /**
         * Construct a post-order comparator with the given node-to-scc mapping
         * and topologically-sorted SCC graph.
         */
    ...
    public Path getPath(Object callee, Number context) {
        ...
        while (!worklist.isEmpty()) {
            ...
            for (Iterator i = navigator.prev(callee).iterator(); i.hasNext();) {
                ...
                if (TRACE_PATH) System.out.println("Edge " + caller + " to "
                    + callee + ": " + r);
                ...
                if (contexts.containsKey(caller)) {
                    ...
                    if (TRACE_PATH) System.out
                        .println("Matches! Adding to worklist.");
                    worklist.add(caller);
